## These functions work together to cache the inverse of a matrix.
## makeCacheMatrix creates a special "matrix" object that can store its inverse,
## and cacheSolve computes (or retrieves) the cached inverse to avoid
## repeated, expensive matrix inversion.

## makeCacheMatrix: creates a special "matrix" object that stores a matrix
## and caches its inverse.

makeCacheMatrix <- function(x = matrix()) {
  inv <- NULL

  set <- function(y) {
    x <<- y
    inv <<- NULL
  }

  get <- function() {
    x
  }

  setInverse <- function(inverse) {
    inv <<- inverse
  }

  getInverse <- function() {
    inv
  }

  list(
    set = set,
    get = get,
    setInverse = setInverse,
    getInverse = getInverse
  )
}

## cacheSolve: computes the inverse of the special "matrix" returned by
## makeCacheMatrix; if the inverse has already been calculated and the
## matrix has not changed, it retrieves the cached inverse.

cacheSolve <- function(x, ...) {
  inv <- x$getInverse()

  if (!is.null(inv)) {
    message("getting cached data")
    return(inv)
  }


  mat <- x$get()

  inv <- solve(mat, ...)

  x$setInverse(inv)

  inv
}

